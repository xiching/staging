...
BASE_DIR = '/tmp/minigo/rl_pipeline_evaluation/'
...
EVALUATE_DIR = os.path.join(BASE_DIR, 'sgf/evaluate')
...
def selfplay(model_name, readouts=10, verbose=2, resign_threshold=0.99):
...
def evaluate(black_model_name, white_model_name):
    """ Evaluate the current best models
    Runs validate on the directories up to the most recent model, or up to
    (but not including) the model specified by `model_num`
    """
    ...
    winner = main.evaluate(
        black_model=black_model, # black is always the best
        white_model=white_model, # white is checkpoint model
...
def rl_loop():
    ...
    for rl_iter in range(MAX_GENERATIONS_PER_PIPELINE):
        ...
        while True:
            selfplay(model_name=model_name)
        ...
        _, latest_mode_name = get_latest_model()
        print("Evaluating the latest model...")
        model_name = evaluate(black_model_name=model_name,
                             white_model_name=latest_mode_name)

...
BASE_DIR = '/tmp/minigo/rl_pipeline/'
...
def selfplay(model_name, readouts=10, verbose=1, resign_threshold=0.99):
...
def rl_loop():
    ...
    for rl_iter in range(5):
        ...
        while len(games) < MAX_GAMES_PER_GENERATION:
            selfplay(model_name=model_name)
            games = gfile.Glob(os.path.join(SELFPLAY_DIR, model_name, '*.zz'))
